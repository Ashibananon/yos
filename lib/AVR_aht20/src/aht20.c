...
static void aht20_send_full_command( uint8_t cmd, uint8_t param1, uint8_t param2 ) {
	iic_stub_start();
	iic_stub_write(AHT20_ADDRESS << 1 | I2C_WRITE);
	iic_stub_write(cmd);
	iic_stub_write(param1);
	iic_stub_write(param2);
	iic_stub_stop();
}
...
static uint8_t aht20_read_cmd() {
	iic_stub_start();
	iic_stub_write(AHT20_ADDRESS << 1 | I2C_READ);
	//register uint8_t read = iic_stub_read(I2C_NACK);
	uint8_t read;
	iic_stub_read(&read, sizeof(read));
	iic_stub_stop();
...
#endif
	iic_stub_read(&aht20, sizeof(aht20));
	iic_stub_stop();
